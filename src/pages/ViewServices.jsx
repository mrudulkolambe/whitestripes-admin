...
const ViewServices = () => {
	...
	const handleSearch = (e) => {
		if (e.target.value.length === 0) {
			...
			setSearchServices(services?.filter((data) => {
				return data?.title.toLowerCase().includes(e.target.value.toLowerCase()) || data?.services?.label?.includes(e.target.value) || data?.description.toLowerCase().includes(e.target.value.toLowerCase())
			}))
		}
	}
							...
							<table className="border-gray-400 rounded-lg overflow-hidden w-full text-sm text-left text-gray-500 dark:text-gray-400">
								<thead className="rounded -t-lg text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
									<tr>
										...
										<th scope="col" colSpan={1} className="px-4 py-3">Service Type</th>
										<th scope="col" colSpan={5} className="px-4 py-3">Description</th>
										<th scope="col" colSpan={1} className="px-4 py-3">Delete</th>
									</tr>
								</thead>
								<tbody>
									{
										searchServices.map((data) => {
											return <tr key={data?.id} className="bg-gray-200 border-b dark:border-gray-700">
												<th colSpan={1} scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><Link className='hover:underline' to={`/edit-services/${data.id}`}>{data?.title}</Link></th>
												<td colSpan={1} className="px-4 py-3"><Link className='hover:underline' to={`/edit-services/${data.id}`}>{data?.services?.label}</Link></td>
												<td colSpan={5} className="px-4 py-3" dangerouslySetInnerHTML={{__html: `${data?.description?.slice(0, 50)}...`}}></td>
